import {
  ...
  export const getAdministrativosController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Error retrieving administrativos', error: error.message });
    }
  };
  ...
  export const getAdministrativoByIdController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Error retrieving administrativo', error: error.message });
    }
  };
  ...
  export const createAdministrativoController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Error creating administrativo', error: error.message });
    }
  };
  ...
  export const updateAdministrativoController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Error updating administrativo', error: error.message });
    }
  };
  ...
  export const deleteAdministrativoController = async (req, res) => {
    try {
      ...
      if (!deletedAdministrativo) {
        return res.status(404).json({ message: 'Administrativo not found' });
      }
      res.status(200).json(deletedAdministrativo);
    } catch (error) {
      res.status(500).json({ message: 'Error deleting administrativo', error: error.message });
    }
  };
